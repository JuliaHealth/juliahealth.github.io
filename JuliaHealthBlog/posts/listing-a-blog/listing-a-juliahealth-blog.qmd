...
Contributions can range from Public Health, Medical-Imaging, Neurobiology, Seizure-detection, Virtual-cell modelling and Epigenetics.

> Juliahealth community has close-knit community of scientists and researchers actively in the medical-field.
![Juliahealth Ghibli vertical image](./juliahealth_vertical.png)
...
*   **Links:** Use `[Text to display](URL)`.
    ```markdown
    Visit the [JuliaHealth GitHub](https://github.com/JuliaHealth).
    ```
*   **Images:** Use `![Alternative text description for accessibility](./image_filename.png)`. Put the image file (e.g., `image_filename.png`) in the *same folder* as your `.qmd` file. The alt text is important!
    ```markdown
    ![Diagram showing the workflow](./workflow-diagram.png)
    *Optional: Add a caption below the image using italics.*
    ```
